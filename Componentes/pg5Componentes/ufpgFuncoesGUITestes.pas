unit ufpgFuncoesGUITestes;
  ...
  * procedures, constantes, funÁıes e vari·veis    *
...
uses
  ...
  uspTestCase, spdxBar, ComCtrls, uspExcecao, ufpgConstanteGUISQLTests, ShellAPI,
...
function IncTime(pTime: TDateTime; Hours, Minutes, Seconds, MSecs: integer): TDateTime;

function IncMinute(pTime: TDateTime; Delta: integer): TDateTime;
//--
...
procedure GravarVariavelExterna(psCampo: string; psValor: string; psSeq: string = '1');

function LerVariavelExterna(psCampo: string; psSeq: string = '1'): string;
...
function GerarTextoAleatorio(piTamanho: integer; pbMin, pbMai, pbNum: boolean): string;

procedure LimparVariaveisExternas;
...
begin
  if Assigned(poPageControl) then
    ...
    raise EMsgAvisoCliente.Create('PageControl n„o disponÌvel para selecionar a aba.');
end;
...
begin
  ...
  // e setando os atributos obrigatÛrios de serem inicializados.
  ...
  begin
    ...
    wShowWindow := SW_SHOWNORMAL; //SW_Hide; Se n„o quiser que apareÁa na tela!
    cbReserved2 := 0;
    lpReserved2 := nil;
  end;

  // Cria o processo e passa por par‚metro o arquivo a ser aberto.
  // Passa tambÈm as estruturas de controle, lStartUpInfo e lProcesso.
  ...
  begin
    ...
    //INFINITE È uma constante porÈm pode ser passado o valor do tempo de espera
...
procedure GravarVariavelExterna(psCampo: string; psValor: string; psSeq: string = '1');
begin
  InserirDadosParaTeste(psSeq, psCampo, psValor);
end;

function LerVariavelExterna(psCampo: string; psSeq: string = '1'): string;
begin
  result := RetornaDadosParaTeste(psSeq, psCampo);
end;
...
begin
  ...
  begin
    ...
    try
      ...
      begin
        ShowMessage('Arquivo de configuraÁ„o n„o encontrado!');
...
begin
  ...
  try
    Sleep(3000); //Necess·rio um tempo atÈ arquivo ser impresso
...
begin
  ...
  //Elimina a flag de erro parta prÛxima suit
  gbErroTest := False;
end;
...
begin
  ...
  try
    ...
    gsBuild := oIni.ReadString('DUNIT', 'BUILD', 'NOTURNO');
  finally
    FreeAndNil(oIni);
  end;
end;
...
function IncTime(pTime: TDateTime; Hours, Minutes, Seconds, MSecs: integer): TDateTime;
begin
  result := pTime + (Hours div 24) + (((Hours mod 24) * 3600000 + Minutes * 60000 +
...
function IncMinute(pTime: TDateTime; Delta: integer): TDateTime;
begin
  result := IncTime(pTime, 0, Delta, 0, 0);
end;
...
const
  ...
  MaiC = 'ABCDEFGHIJKLMNOPQRSTUVWXYZ';
  NumC = '1234567890';
var
  p, q: integer;
  char, Texto: string;
...
procedure LimparVariaveisExternas;
begin
  ExecutarLimparVariaveisExternas;
end;
